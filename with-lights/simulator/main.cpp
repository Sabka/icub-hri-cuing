...
int main(int argc, char *argv[])
{
    ...
    struct termios tty;
    SerialPort port = SerialPort("/dev/ttyUSB0", 115200);

    Network yarp;
    if (!yarp.checkNetwork())
    {
        throw runtime_error("Error: yarp server not available\n");
    }
    ...
    if (!params.check("robot"))
    {
        ...
        throw runtime_error("Error: robot name missing.\nPlease specify the name of the robot\n--robot <name> (e.g. icubSim)\n");
    }
    ...
    if (!ok)
    {
        throw runtime_error("Problems acquiring interfaces\n");
    }
    ...
    while(conf_num < confs.size())
    {
        ...
        // Turn off led diodes light
        /*switch (c.second)
        {
            case side::left:
                Time::delay(LIGHT_ON_TIME);
                port.write("3");
                break;
